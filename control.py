...
from sonar_display import update_distance, update_img, show_sonar, temporal_check
...
while(1):
    ...
    for i in range(repeat):
        ...
        for j in range(len(new_message)):
            fileObject.write(str(new_message[j])+" ")  
        fileObject.write("\n")  
        sonar_img=update_distance(sonar_img, new_message)
    ...
    temporal_check(sonar_img, freq, vertical_span, bins_freq, 4)
